...
PRIVATE struct inode * create_file(char * path, int flags, int inode_mode);
PRIVATE int alloc_imap_bit(int dev);
PRIVATE int alloc_smap_bit(int dev, int nr_sects_to_alloc);
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect, int inode_mode);
...
PUBLIC int do_open_dir()
{
    ...
    if (flags & O_CREAT) {
        if (inode_nr) {
            ...
            pin = create_file(pathname, flags, I_DIRECTORY);
        }
    }
    else {
        assert(flags & O_RDWR);
...
PUBLIC int do_close_dir()
{
    __pathCount = __pathCount > 0 ? __pathCount - 1: 0; 
    memset(__path[__pathCount], 0, sizeof(__path[__pathCount]));
}
...
PUBLIC int do_open()
{
    ...
    if (flags & O_CREAT) {
        if (inode_nr) {
            ...
            pin = create_file(pathname, flags, I_REGULAR);
        }
    }
    else {
        assert(flags & O_RDWR);
